...
settingsRouter.put("/", zValidator("json", settingsSchema), async (c) => {
  ...
  const saved = await withTransaction(async (client) => {
    const result = await client.query(`UPDATE organizations SET
      ...
      receipt_default_notes=$21, receipt_default_phrase=$22, document_settings_version=document_settings_version+1, updated_at=NOW()
      WHERE user_id=$23 AND deleted_at IS NULL RETURNING id`, [
      ...
      body.quotation_default_notes || null, body.receipt_default_notes || null, body.receipt_default_phrase || null, userId])
