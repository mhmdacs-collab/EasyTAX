...
receiptsRouter.post("/:id/cancel",zValidator("json",cancelSchema),async(c)=>{
  ...
  const result=await withTransaction(async(client)=>{
    ...
    const receiptDate=dateOnly(row.receipt_date)
    const periodLock=await activePeriodLock(client,orgId,receiptDate)
    if(periodLock)return {error:"PERIOD_LOCKED" as const,message:lockedPeriodMessage(periodLock)}
    await reverseSourceJournalEntries(client,{organizationId:orgId,sourceType:"receipt",sourceId:String(row.id),reversalDate:receiptDate,reason})
