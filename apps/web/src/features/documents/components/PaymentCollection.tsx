...
export function PaymentCollection({form,methods,enabled,onEnabled,payments,onPayments}:{form:UseFormReturn<DocumentFormData>;methods:string[];enabled:boolean;onEnabled:(value:boolean)=>void;payments:CollectedPayment[];onPayments:(value:CollectedPayment[])=>void}){
  ...
  return <div className="space-y-3">
    <label className="flex cursor-pointer items-center gap-2 font-medium"><input type="checkbox" checked={enabled} onChange={(event)=>{ onEnabled(event.target.checked); }}/>تسجيل مبلغ محصل</label>
    {enabled?<div className="space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2 rounded bg-background p-3 text-sm"><span>المبلغ المحصل: <strong dir="ltr">{formatCurrency(collected)}</strong></span><span>المبلغ المستحق: <strong dir="ltr" className={collected>total?"text-destructive":""}>{formatCurrency(Math.max(0,total-collected))}</strong></span></div>
