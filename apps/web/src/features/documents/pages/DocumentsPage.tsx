...
type Filter="all"|"invoice"|"quotation"|"adjustment"|"receipt"
type ListedItem={id:string;kind:"invoice"|"quotation"|"credit_note"|"debit_note"|"receipt";number:string;party:string;date:string;amount:number;status:string}
...
export function DocumentsPage(){
 ...
 const items=useMemo<ListedItem[]>(()=>[
  ...documents.map((document)=>({id:document.id,kind:document.type,number:document.number,party:document.customer_snapshot?.name??"—",date:document.issue_date,amount:Number(document.total),status:document.status})),
  ...receipts.map((receipt)=>({id:receipt.id,kind:"receipt" as const,number:receipt.number,party:receipt.payer_name,date:receipt.receipt_date,amount:Number(receipt.amount),status:receipt.status})),
 ].sort((a,b)=>b.date.localeCompare(a.date)),[documents,receipts])
 const shown=useMemo(()=>items.filter((item)=>(filter==="all"||item.kind===filter||(filter==="adjustment"&&(item.kind==="credit_note"||item.kind==="debit_note")))&&(!search||item.number.toLocaleLowerCase("ar").includes(search.toLocaleLowerCase("ar"))||item.party.toLocaleLowerCase("ar").includes(search.toLocaleLowerCase("ar")))),[items,filter,search])
 ...
 <div className="flex flex-wrap gap-2">{([['all','الكل'],['invoice','الفواتير'],['quotation','عروض الأسعار'],['adjustment','الإشعارات الدائنة والمدينة'],['receipt','سندات القبض']] as const).map(([value,label])=><Button key={value} type="button" size="sm" variant={filter===value?"default":"outline"} onClick={()=>{setFilter(value)}}>{label}</Button>)}</div>
...
function Item({item}:{item:ListedItem}){const label=item.kind==="quotation"?"عرض سعر":item.kind==="receipt"?"سند قبض":item.kind==="credit_note"?"إشعار دائن":item.kind==="debit_note"?"إشعار مدين":"فاتورة";return <><span>{label}{item.status==="cancelled"?<small className="ms-2 text-destructive">ملغى</small>:null}</span><span className="font-mono font-medium">{item.status==="draft"?"مسودة":item.number}</span><span>{item.party}</span><span className="text-muted-foreground">{formatDate(item.date)}</span><span>{formatCurrency(item.amount)}</span></>}
