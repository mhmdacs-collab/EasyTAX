...
type ListedItem={id:string;kind:"invoice"|"quotation"|"receipt";number:string;party:string;date:string;amount:number;status:string}
...
function Item({item}:{item:ListedItem}){return <><span>{item.kind==="quotation"?"عرض سعر":item.kind==="receipt"?"سند قبض":"فاتورة"}{item.status==="cancelled"?<small className="ms-2 text-destructive">ملغى</small>:null}</span><span className="font-mono font-medium">{item.status==="draft"?"مسودة":item.number}</span><span>{item.party}</span><span className="text-muted-foreground">{formatDate(item.date)}</span><span>{formatCurrency(item.amount)}</span></>}
