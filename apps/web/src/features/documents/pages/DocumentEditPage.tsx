...
export function DocumentEditPage() {
  ...
  useEffect(() => { void fetchDocument(id).then((result) => { setDocument(result.document) }) }, [id])
  if (!document) return <p className="p-6 text-muted-foreground">جاري تحميل المسودة...</p>
  if (document.status !== "draft") return <p className="p-6 text-muted-foreground">يمكن تعديل المسودات فقط.</p>
  const customer = document.customer_snapshot
  const draft: Document = {
    id:document.id, organization_id:"", type:document.type === "quotation" ? "quotation" : "tax_invoice", status:"draft", number:document.number,
