...
export default function SettingsPage() {
  ...
  useEffect(() => { void fetchSettings().then((data) => {
    const o=data.organization, seq=Object.fromEntries(data.sequences.map((s) => [s.document_type, Number(s.next_number)]))
    setForm({ ...o, payment_methods:data.payment_methods, quotation_terms:data.quotation_terms.map((t)=>t.text), invoice:seq.invoice||1, quotation:seq.quotation||1, receipt:seq.receipt||1 } as Form)
  }).catch((e:Error)=>toast({title:"تعذر تحميل الإعدادات",description:e.message,variant:"error"})) }, [])
  if (!form) return <div className="p-6 text-muted-foreground">جاري تحميل الإعدادات...</div>
  const set=(key:string,value:string|boolean|number)=>setForm((current)=>current ? ({...current,[key]:value} as Form) : current)
  const field=(key:string,label:string, options?:{readOnly?:boolean;required?:boolean;dir?:"ltr"})=><div className="space-y-1.5">
    <Label htmlFor={key}>{label}{options?.required ? " *" : ""}</Label>
    <Input id={key} value={String(form[key]??"")} readOnly={options?.readOnly} required={options?.required} dir={options?.dir} className={options?.readOnly?"bg-muted":""} onChange={(e)=>set(key,e.target.value)} />
  </div>
  const submit=async()=>{setSaving(true);try{await saveSettings({
    ...
    sequences:{invoice:Number(form.invoice),quotation:Number(form.quotation),receipt:Number(form.receipt)},
