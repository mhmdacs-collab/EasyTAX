...
export function ScanPurchasePage() {
  ...
  async function scanImage(file?: File) {
    ...
    try { const result=await new BrowserQRCodeReader().decodeFromImageUrl(url); acceptRaw(result.getText()) }
  ...
  return <div className="mx-auto max-w-3xl space-y-5 p-4 sm:p-6">
    ...
    {!decoded?<Card><CardContent className="space-y-5 pt-6"><div className="overflow-hidden rounded-xl bg-black"><video ref={videoRef} className={`aspect-video w-full object-cover ${scanning?"block":"hidden"}`} muted playsInline/>{!scanning?<div className="flex aspect-video items-center justify-center"><Camera className="size-16 text-white/40"/></div>:null}</div><div className="grid gap-2 sm:grid-cols-2"><Button size="lg" onClick={() => void startCamera()} disabled={scanning}><Camera className="me-2 size-5"/>{scanning?"جاري المسح…":"بدء المسح"}</Button><Button size="lg" variant="outline" asChild><label className="cursor-pointer"><ImagePlus className="me-2 size-5"/>اختيار صورة<input type="file" accept="image/*" className="hidden" onChange={(event)=>void scanImage(event.target.files?.[0])}/></label></Button></div></CardContent></Card>:
