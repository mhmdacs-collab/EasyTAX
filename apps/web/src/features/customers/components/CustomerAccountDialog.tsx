...
import { createCustomerReceipt, fetchCustomerAccount, fetchSettings, type CentralCustomer, type CustomerAccount } from "@/lib/platform/api"
...
export function CustomerAccountDialog({customer,onClose}:{customer:CentralCustomer;onClose:()=>void}){
  ...
  useEffect(()=>{void load();void fetchSettings().then((settings)=>{setMethods(settings.payment_methods.filter((item)=>item.is_active).map((item)=>item.name))})},[load])
  const receive=async()=>{const value=Number(amount);if(value<=0||!method){toast({title:"أدخل المبلغ واختر طريقة السداد",variant:"error"});return}setSaving(true);try{const result=await createCustomerReceipt(customer.id,{amount:value,payment_method_name:method,receipt_date:date,reference_number:reference||undefined,notes:notes||undefined});toast({title:`تم إصدار سند القبض رقم ${result.receipt.number}`,variant:"success"});setAmount("");setReference("");setNotes("");setReceiving(false);await load()}catch(error){toast({title:"تعذر تسجيل الدفعة",description:error instanceof Error?error.message:"حاول مرة أخرى",variant:"error"})}finally{setSaving(false)}}
