...
import { Download, Mail, MessageCircle, Printer, Share2, XCircle } from "lucide-react"
import { cancelReceipt, fetchBrandingAssetUrl, fetchReceipt, type BrandingAssetKind, type CentralReceipt } from "@/lib/platform/api"
...
export function ReceiptViewPage(){
  ...
  const cancel=async()=>{const reason=window.prompt("سبب إلغاء وعكس سند القبض");if(!reason?.trim())return;try{const result=await cancelReceipt(id,reason.trim());setReceipt(result.receipt);toast({title:"تم إلغاء وعكس سند القبض",variant:"success"})}catch(error){toast({title:"تعذر إلغاء السند",description:error instanceof Error?error.message:"حاول مرة أخرى",variant:"error"})}}
  ...
  return <div className="min-h-screen bg-muted/20 p-3 sm:p-6" dir="rtl">
    <div className="mx-auto mb-3 flex max-w-3xl flex-wrap items-center justify-between gap-2 print:hidden"><Link to="/documents" className="text-sm text-muted-foreground">العودة للمستندات</Link><div className="flex flex-wrap gap-2">{receipt.status==="issued"?<Button variant="outline" className="text-destructive" onClick={()=>{void cancel()}}><XCircle className="size-4"/>إلغاء وعكس السند</Button>:null}<Button variant="outline" onClick={()=>{window.print()}}><Printer className="size-4"/>طباعة / PDF</Button><Button variant="outline" disabled={creatingPdf} onClick={()=>{void download()}}><Download className="size-4"/>تنزيل PDF</Button>{receipt.status==="issued"?<><Button variant="outline" disabled={creatingPdf} onClick={()=>{void nativeShare()}}><Share2 className="size-4"/>مشاركة PDF</Button>{emailUrl?<Button variant="outline" disabled={creatingPdf} onClick={()=>{void openWithPdf('email')}}><Mail className="size-4"/>البريد</Button>:null}{whatsappUrl?<Button variant="outline" disabled={creatingPdf} onClick={()=>{void openWithPdf('whatsapp')}}><MessageCircle className="size-4"/>واتساب</Button>:null}</>:null}</div></div>
    <article ref={printRef} data-print-area className="mx-auto max-w-3xl rounded-xl border bg-white p-6 shadow-sm sm:p-10 print:border-0 print:shadow-none">
      <header className="flex items-start justify-between gap-4"><div><h1 className="text-3xl font-bold">سند قبض</h1><p className="mt-1 font-mono text-primary">{receipt.number}</p>{receipt.status==="cancelled"?<p className="mt-2 font-bold text-destructive">ملغى ومعكوس</p>:null}</div><div className="flex items-start gap-4">{assets.logo?<img src={assets.logo} alt="شعار المنشأة" className="h-[5.5rem] w-[8.8rem] object-contain"/>:null}<div className="text-end text-sm"><p>{formatDate(receipt.receipt_date)}</p><p className="text-muted-foreground">{receipt.status==="cancelled"?"ملغى":"صادر"}</p></div></div></header>
      ...
      {receipt.status==="cancelled"?<div className="mt-6 rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive"><strong>سبب الإلغاء:</strong> {receipt.cancellation_reason}</div>:null}
